...
• Navigating directories and editing Terraform files with nano 
• Terraform workflow: plan → apply → state list → destroy 
• How Terraform wires AWS components together (VPC → subnet → IGW → routes → SG → EC2) 
• Handling a real-world error (invalid AMI) and how to fix it using a data source 
